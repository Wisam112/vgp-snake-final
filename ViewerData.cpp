...
void igl::opengl::ViewerData::copy_options(ViewerCore& from, ViewerCore& to, ViewerData& m)
{
	to.set(m.show_overlay, from.is_set(m.show_overlay));
	to.set(m.show_overlay_depth, from.is_set(m.show_overlay_depth));
	to.set(m.show_texture, from.is_set(m.show_texture));
	to.set(m.show_faces, from.is_set(m.show_faces));
	to.set(m.show_lines, from.is_set(m.show_lines));
}
...
void igl::opengl::ViewerData::animate()
{
	if (velco[0] == 0 && velco[1] == 0 && velco[2] == 0)
		return;

	MyTranslate(velco);
	MyRotate(velco.normalized(), 1.f * velco.maxCoeff());
}
Eigen::Matrix4f igl::opengl::ViewerData::MakeTrans()
{
	if (Parent == nullptr)
		return T.matrix() * R.matrix() * T1.matrix() * S.matrix();
	else return Parent->MakeTrans_noscale() * T1.matrix() * R.matrix() * T.matrix() * S.matrix();
}

Eigen::Matrix4f igl::opengl::ViewerData::MakeTrans_noscale()
{
	if (Parent == nullptr)
		return T.matrix() * R.matrix() * T1.matrix();
	else return Parent->MakeTrans_noscale() * T1.matrix() * R.matrix() * T.matrix();
}
Eigen::Vector3f igl::opengl::ViewerData::getTop() {
	// Find the bounding box
	Eigen::Vector3d m = V.colwise().minCoeff();
	Eigen::Vector3d M = V.colwise().maxCoeff();
	Eigen::RowVector3d getTop = Eigen::RowVector3d(((M(0) + m(0))) / 2, M(1), (((m(2) + M(2))) / 2));
	Eigen::Vector4f oldtop = Eigen::Vector4f(getTop.x(), getTop.y(), getTop.z(), 1);
	Eigen::Vector4f res = MakeTrans() * oldtop;
	return Eigen::Vector3f(res[0], res[1], res[2]);
}

Eigen::Vector3f igl::opengl::ViewerData::getBottom() {
	// Find the bounding box
	Eigen::Vector3d m = V.colwise().minCoeff();
	Eigen::Vector3d M = V.colwise().maxCoeff();
	Eigen::RowVector3d getTop = Eigen::RowVector3d(((M(0) + m(0))) / 2, m(1), (((m(2) + M(2))) / 2));
...
Eigen::Vector3f igl::opengl::ViewerData::getCeneter() {
	Eigen::Vector3d m = V.colwise().minCoeff();
	Eigen::Vector3d M = V.colwise().maxCoeff();
	Eigen::RowVector3d getTop = Eigen::RowVector3d(((M(0) + m(0))) / 2, (M(1) + m(1)) / 2, (((m(2) + M(2))) / 2));
	Eigen::Vector4f oldtop = Eigen::Vector4f(getTop.x(), getTop.y(), getTop.z(), 1);
	Eigen::Vector4f res = MakeTrans() * oldtop;
	return Eigen::Vector3f(res[0], res[1], res[2]);
}

void igl::opengl::ViewerData::Rotate_velo(Eigen::Vector3f rotAxis, float angle)
{
	Eigen::Transform<float, 3, Eigen::Affine>  R = Eigen::Transform<float, 3, Eigen::Affine>::Identity();
	R.rotate(Eigen::AngleAxisf(angle, rotAxis));
	Eigen::Vector4f _velo = R.matrix() * Eigen::Vector4f(velco[0], velco[1], velco[2], 0);
	velco = Eigen::Vector3f(_velo[0], _velo[1], _velo[2]);
}
void igl::opengl::ViewerData::Random_velco(int lvl)
{
	float x, y;
	x = RandomFloat(-0.02, 0.02);
	y = RandomFloat(-0.02, 0.02);
	velco = Eigen::Vector3f((x + 0.001) * lvl, (y + 0.001) * lvl, 0);
}

void igl::opengl::ViewerData::drawAxis(int t) {
	Eigen::Vector3d m = V.colwise().minCoeff();
	Eigen::Vector3d M = V.colwise().maxCoeff();
	Eigen::RowVector3d getBottom = Eigen::RowVector3d(((m(0) + M(0)) / 2), m(1), (m(2) + M(2)) / 2);
	Eigen::MatrixXd V_points(1, 3);
	Eigen::RowVector3d getTop = Eigen::RowVector3d(((M(0) + m(0))) / 2, M(1), (((m(2) + M(2))) / 2));
	V_points <<
		getBottom;
	Eigen::MatrixXd V_points1(1, 3);
	if (t == 1)
	{

		V_points1 << getTop;
		add_points(V_points1, Eigen::RowVector3d(0, 0, 1));
		//add_edges(V_points1.row(0), V_points1.row(1), Eigen::RowVector3d(0, 1, 0));

	}

	Eigen::MatrixXd E_points1(2, 3);
	E_points1 <<
		((m(0) + M(0)) / 2) + 1.5, M(1), (((M(2) + m(2))) / 2),
		(((m(0) + M(0))) / 2) - 1.5, M(1), (((M(2) + m(2))) / 2);
	Eigen::MatrixXd E_points2(2, 3);
	E_points2 <<
		((m(0) + M(0)) / 2), M(1), (((M(2) + m(2))) / 2) + 1,
		(((m(0) + M(0))) / 2), M(1), (((M(2) + m(2))) / 2) - 1;
	Eigen::MatrixXd E_points3(2, 3);
	E_points3 << ((m(0) + M(0)) / 2), M(1) - 1.2, (((M(2) + m(2))) / 2),
		(((m(0) + M(0))) / 2), M(1) + 1.2, (((M(2) + m(2))) / 2);


	add_points(V_points, Eigen::RowVector3d(0.59, 0.29, 0.00));
	//add_points(E_points1, Eigen::RowVector3d(1, 0, 1));
	add_edges(E_points1.row(0), E_points1.row(1), Eigen::RowVector3d(0.59, 0.29, 0.2));
	//add_points(E_points2, Eigen::RowVector3d(0, 0, 1));
	add_edges(E_points2.row(0), E_points2.row(1), Eigen::RowVector3d(0.79, 0.29, 0.00));
	add_edges(E_points3.row(0), E_points3.row(1), Eigen::RowVector3d(0.59, 0.39, 0.00));
}
void igl::opengl::ViewerData::random_place()
{
	float x, y, z;
	x = RandomFloat((-1 * ABS_X), (1 * ABS_X));
	y = RandomFloat((-1 * ABS_Y), (1 * ABS_Y));
	z = RandomFloat(-1, 1);
	MyTranslate(Eigen::Vector3f(x + 0.001, y, z));
}
float igl::opengl::ViewerData::RandomFloat(float a, float b) {
